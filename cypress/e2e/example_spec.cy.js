...
describe("pipe()", () => {
  ...
  context("when passed a synchronous function", () => {
    ...
    it("should show the function name in the Command Log", (done) => {
      ...
      const timeout = setTimeout(() => {
        done(new Error("Timed out waiting for log"));
      }, 1000);
      cy.on("log:added", (attrs, log) => {
        if (log.get("name") === "pipe") {
          ...
          expect(log.invoke("consoleProps")).to.have.property(
            "Command",
            "pipe"
          );
      ...
      cy.on("log:changed", (attrs, log) => {
        if (log.get("name") === "pipe") {
          ...
          expect(log.get("message")).to.eq(
            'getProp("foo", 1, function, false, [1,"foo"], {"foo":"bar"})'
          );
    ...
    context("when visiting a page", () => {
      ...
      it("should fail with the selector in the error message if element is not found", (done) => {
        ...
        cy.get("body").pipe(thisWillFail, { timeout: 50 });
      });

      it("should not fail with a missing selector when non-existance assertion is used", () => {
        const thisWillFail = ($el) => $el.find("#wontfind");
        cy.get("body").pipe(thisWillFail, { timeout: 50 }).should("not.exist");
      });
      ...
      it("should use the body tag if no previous subject was provided", () => {
        cy.pipe(getFirst).pipe(getSecond).should("contain", "foobar");
      });

      it("should use the withinSubject if it is defined", () => {
        cy.pipe(getFirst).within(() => {
          cy.pipe(($el) => $el).should("have.attr", "id", "first");
        });
      });
    });
  });
  ...
  context("when passed a function that uses cy commands", () => {
    ...
    it("should show the function name in the Command Log", (done) => {
      ...
      cy.wrap({ foo: "bar" }).pipe(getFoo); // command log should show 'getFoo'
    });
        ...
        cy.get("body")
          ...
          .then(($el) => {
            ...
            if (snapshots) {
              ...
              expect(lastLog.get("snapshots")[0]).to.have.property(
                "name",
                "before"
              );
              expect(lastLog.get("snapshots")[1]).to.have.property(
                "name",
                "after"
              );
            }
